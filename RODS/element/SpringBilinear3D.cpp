#include "SpringBilinear3D.h"

SpringBilinear3D::SpringBilinear3D(const int id, Node * nodeI, Node * nodeJ, const double k0, const double uy,
									const double alpha, RODS::LocalAxis axis) :
	ROD3D(id, nodeI, nodeJ), k0(k0), uy(uy),
...
SpringBilinear3D::~SpringBilinear3D()
{
}

void SpringBilinear3D::buildMatrix()
{
	K = T.t()*k*T;
}

void SpringBilinear3D::getResponse(const bool update)
{
...
void SpringBilinear3D::assembleNonlinearForceVector(vec &q)
{
...
void SpringBilinear3D::assembleStiffnessMatrix(mat &K)
{
	int local[6] = { 0,1,2,3,4,5 };
	int global[6] = { nodeI->dofX->eqnId, nodeI->dofY->eqnId, nodeI->dofZ->eqnId,
					  nodeJ->dofX->eqnId, nodeJ->dofY->eqnId, nodeJ->dofZ->eqnId };
