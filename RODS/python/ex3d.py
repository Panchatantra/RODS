...
# 添加线性阻尼器
# c = 1.0
# RODS.add_dashpot_2d(101, 6, 9, c, RODS.LocalAxis.U1.value)
# RODS.add_dashpot_2d(102, 6, 11, c, RODS.LocalAxis.U1.value)
# RODS.add_dashpot_2d(103, 8, 11, c, RODS.LocalAxis.U1.value)

# 添加非线性阻尼器
# k = 100.0
# c = 5.0
# alpha = 0.15
# U = RODS.LocalAxis.U1.value
# RODS.add_dashpot_maxwell_2d(101, 6, 9, k, c, alpha, U)
# RODS.add_dashpot_maxwell_2d(102, 6, 11, k, c, alpha, U)
# RODS.add_dashpot_maxwell_2d(103, 8, 11, k, c, alpha, U)
...
alpha = 0.02
U = RODS.LocalAxis.U1.value
RODS.add_spring_bilinear_2d(101, 6, 9, k0, uy, alpha, U)
RODS.add_spring_bilinear_2d(102, 6, 11, k0, uy, alpha, U)
RODS.add_spring_bilinear_2d(103, 8, 11, k0, uy, alpha, U)
...
# 添加自由度和单元响应记录器
dof_recorder_id = 0
RODS.add_dof_recorder(dof_recorder_id, RODS.Response.DISP.value, 'top_disp_frame_damped.txt'.encode())
# 将节点4的X方向自由度加入记录器
node_id = 4
RODS.add_dof_to_recorder(3*(node_id-1)+1, dof_recorder_id)
# 添加阻尼器单元响应记录器
ele_recorder_id = 0
RODS.add_ele_recorder(ele_recorder_id, RODS.Response.ALL.value, 'damper_102.txt'.encode())
RODS.add_ele_to_recorder(102, ele_recorder_id)
...
# 读取位移响应结果并绘制曲线
data_d = np.loadtxt('top_disp_frame_damped.txt')
t = data_d[:,0]
d = data_d[:,1]
...
# 读取阻尼器响应结果并绘制曲线
data_d = np.loadtxt('damper_102.txt')
d = data_d[:,1]
f = data_d[:,2]
